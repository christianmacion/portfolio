...
export const ResendEventSchema = z
  .object({
    ...
    svix_timestamp: z.union([z.string().regex(/^\d{10,20}$/), z.number().int().min(1_000_000_000).max(99_999_999_999)]),
    data: z.object({
...
export const RESEND_WEBHOOK_DURABLE_EFFECT = 'delivery event + outbox/contact state update' as const;
export const RESEND_WEBHOOK_BUDGET = { minute: 120, hour: 600, scope: 'provider_route' } as const;
export const RESEND_WEBHOOK_AUTH_GATE = 'Resend/Svix signature over raw bytes via svix-id / svix-timestamp / svix-signature (HMAC-SHA256 of `${id}.${ts}.${raw}`) + replay dedupe on provider_event_id' as const;
