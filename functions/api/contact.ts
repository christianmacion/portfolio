...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!key || !(await sha256(key)).length)
  ...
  const existing = await env.DB.prepare(
    'SELECT public_id, body_hash FROM contact_submissions WHERE idempotency_key_hash = ?',
  )
    .bind(idempotencyHash)
    .first<{ public_id: string; body_hash: string }>();
  if (existing) {
    if (existing.body_hash !== bodyHash)
      return errorResponse(rid, 409, 'IDEMPOTENCY_CONFLICT', 'idempotency_conflict');
    return json<ContactAccepted>(
      {
        ok: true,
        request_id: rid,
        submission_ref: existing.public_id,
        notification: 'queued',
        duplicate: true,
      },
      200,
  ...
  if (env.TURNSTILE_SECRET_PRIMARY) {
    const verify = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', {
      ...
      body: new URLSearchParams({
        secret: env.TURNSTILE_SECRET_PRIMARY,
  ...
  const insert = await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO contact_submissions (public_id, request_id, idempotency_key_hash, body_hash, name, email, subject, message, ip_hash, user_agent, turnstile_verified) SELECT ?,?,?,?,?,?,?,?,?,?,1 WHERE (SELECT COUNT(*) FROM contact_submissions WHERE ip_hash=? AND created_at >= ((unixepoch()/3600)*3600)) < 5 ON CONFLICT(idempotency_key_hash) DO NOTHING`,
    ).bind(
      publicId,
      rid,
      idempotencyHash,
    ...
    env.DB.prepare(
      'INSERT INTO notification_outbox (contact_id, dedupe_key) SELECT id, ? FROM contact_submissions WHERE public_id = ? ON CONFLICT(dedupe_key) DO NOTHING',
    ).bind(dedupe, publicId),
  ]);
  if (!insert[0].meta.changes) return errorResponse(rid, 429, 'RATE_LIMIT', 'rate_limit');
  await env.RATELIMIT.put(
    limitKey,
    JSON.stringify({ count: (cached?.count ?? 0) + 1, reset_at: (hour + 1) * 3600 }),
    { expirationTtl: 3900 },
  );
  return json<ContactAccepted>(
    {
