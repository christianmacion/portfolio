...
import {
  CONTACT_MARK_READ_AUTH_GATE,
  CONTACT_MARK_READ_BUDGET,
  CONTACT_MARK_READ_DURABLE_EFFECT,
} from '../../../types/contact-mark-read';
import {
  allowedOrigin,
  boundedJson,
  constantTimeEqual,
...
export const onRequestPatch: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  if (request.headers.get('Content-Type')?.split(';')[0] !== 'application/json')
    return errorResponse(rid, 415, 'VALIDATION_ERROR', 'validation_error');
  ...
  if (!csrfHeader || csrfHeader.length > 256)
    return errorResponse(rid, 403, 'CSRF_INVALID', 'csrf_invalid');
  ...
  if (subjectHash.slice(0, 8) !== subjectTail)
    return errorResponse(rid, 403, 'CSRF_INVALID', 'csrf_invalid');
  const expectedMac = await hmacSha256(csrfSecret, `${subjectHash}.${ts}.${rid}`);
  if (!constantTimeEqual(mac, expectedMac))
    return errorResponse(rid, 403, 'CSRF_INVALID', 'csrf_invalid');
  if (env.CSRF_SIGNING_KEY_PREVIOUS) {
    const expectedMacPrev = await hmacSha256(
      env.CSRF_SIGNING_KEY_PREVIOUS,
      `${subjectHash}.${ts}.${rid}`,
    );
    if (!constantTimeEqual(mac, expectedMacPrev))
      return errorResponse(rid, 403, 'CSRF_INVALID', 'csrf_invalid');
  }
  ...
  if (Number(minCount ?? '0') >= CONTACT_MARK_READ_BUDGET.minute)
    return errorResponse(rid, 429, 'RATE_LIMIT', 'rate_limit');
  if (Number(hourCount ?? '0') >= CONTACT_MARK_READ_BUDGET.hour)
    return errorResponse(rid, 429, 'RATE_LIMIT', 'rate_limit');
  ...
  const existing = await env.DB.prepare(
    'SELECT read_at FROM contact_submissions WHERE public_id = ?',
  )
    .bind(publicId)
  ...
  const updateResult = await env.DB.batch([
    env.DB.prepare('UPDATE contact_submissions SET read_at = ? WHERE public_id = ?').bind(
      newReadAt,
      publicId,
    ),
    env.DB.prepare(
      ...
      dedupeKey,
      subjectHashForBudget,
      action,
      publicId,
      rid,
      JSON.stringify({ source: 'pages_function', idem: idemKey }),
    ),
  ]);
  ...
  const responseBody: MarkReadResponse = {
    ok: true,
    request_id: rid,
    id: publicId,
    read_at: newReadAt,
  ...
  return new Response(null, {
    ...
    headers: {
      ...
      'Access-Control-Allow-Headers':
        'Authorization, Content-Type, Idempotency-Key, X-CSRF-Token, Sec-Fetch-Site',
...
export const _markReadContractRef = {
  auth_gate: CONTACT_MARK_READ_AUTH_GATE,
  budget: CONTACT_MARK_READ_BUDGET,
  durable_effect: CONTACT_MARK_READ_DURABLE_EFFECT,
} as const;
