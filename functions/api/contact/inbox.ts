...
import { CONTACT_INBOX_AUTH_GATE, CONTACT_INBOX_BUDGET, CONTACT_INBOX_DURABLE_EFFECT } from '../../types/contact-inbox';
import { errorResponse, hmacSha256, json, parseAccessJwt, parseInboxQuery, requestId, sha256, type Env, type InboxItem, type InboxResponse } from '../../lib/contracts';
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  if (env.CF_ACCESS_TEAM_DOMAIN && !claims.iss.endsWith(`.${env.CF_ACCESS_TEAM_DOMAIN}`) && claims.iss !== env.CF_ACCESS_TEAM_DOMAIN) {
  ...
  if (!ALLOWED_STATES.has(query.state)) return errorResponse(rid, 422, 'VALIDATION_ERROR', 'validation_error');
  ...
  if (Number(minCount ?? '0') >= CONTACT_INBOX_BUDGET.minute) return errorResponse(rid, 429, 'RATE_LIMIT', 'rate_limit');
  if (Number(hourCount ?? '0') >= CONTACT_INBOX_BUDGET.hour) return errorResponse(rid, 429, 'RATE_LIMIT', 'rate_limit');
  ...
  const items: InboxItem[] = rows.map((row) => ({
    ...
    notification_status: (['pending', 'sent', 'delivered', 'failed', 'bounced'].includes(row.notification_status) ? row.notification_status : 'pending') as InboxItem['notification_status'],
  }));
...
export const _inboxContractRef = { auth_gate: CONTACT_INBOX_AUTH_GATE, budget: CONTACT_INBOX_BUDGET, durable_effect: CONTACT_INBOX_DURABLE_EFFECT } as const;
